...
- Do NOT flag a correctly-wired LED (one whose anode net contains a resistor) as missing a current-limiting resistor. This applies regardless of how the fault is categorized ("power", "component", "signal", etc.) or how it is phrased (e.g. "power connection issue", "excessive current risk", "LED protection"). If an R-prefix component is on the same net as the LED anode, a current-limiting resistor is present — do NOT generate any fault about it.
